...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'weirdplace':
				{
						defaultCamZoom = 0.7;
						curStage = 'weirdplace';
						var bg:FlxSprite = new FlxSprite(-950, -600).loadGraphic(Paths.image('weirdplace'));
						bg.antialiasing = true;
						...
						var stageFront:FlxSprite = new FlxSprite(-1400, 600).loadGraphic(Paths.image('trackfloor'));
						//stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
						...
						realhorse = new FlxSprite(-1000, 400);
						realhorse.frames = Paths.getSparrowAtlas('characters/RealHorse');
						realhorse.animation.addByPrefix('bop', "RealHorse instance 1", 24, false);
						realhorse.antialiasing = true;
						realhorse.updateHitbox();
						if(FlxG.save.data.distractions){
							add(realhorse);
						}
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - FNFAimkid100Gecs": ""), 16);
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				var songLowercase = StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase();
				if (songLowercase=="stupid-horse")
					{
						FlxG.switchState(new FunnyCredits());
					}
		...
		switch (curStage)
		{
			...
			case 'weirdplace':
			{
				...
				if (curBeat % 2 == 0)
					{
						if(FlxG.save.data.distractions){
								realhorse.animation.play('bop', true);
						}
					}
			}
			case "philly":
